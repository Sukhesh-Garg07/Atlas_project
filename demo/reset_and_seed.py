...
What it does:
  1. Wipes every application table (assets, records, edges, connectors,
     documents, extractions, workflow history, review queueâ¦), so you start the
...
Run (backend must be up - default :8010, override with ATLAS_API). Easiest - the
wrapper works from any folder:
...
Notes:
  * DATABASE_URL is read from backend/.env. API defaults to http://127.0.0.1:8010.
...
API = os.environ.get("ATLAS_API", "http://127.0.0.1:8010")
...
def _check_backend() -> bool:
    try:
        ...
        print("  Start it:  cd backend ; .\\.venv\\Scripts\\python.exe -m uvicorn app.main:app --host 127.0.0.1 --port 8010")
        return False
...
def seed_pdf(path: str) -> None:
    ...
    print(f"  job {job_id} started ({started.get('label')}). Tailingâ¦")
